...
void MainMenu::PerformLabelsMenuCommands(COMMAND command) {
	...
	case ID_EQUATES_CLEAR_SYSTEM_EQUATES:
		::g_EquateListLogic->Clear(*::g_Workspace->GetSystemEquateList());
		return;

	case ID_EQUATES_DISPLAY_SYSTEM_EQUATES:
		::g_EquateListLogic->Edit(*::g_Workspace->GetSystemEquateList(), false, L"");
		return;

	case ID_EQUATES_CLEAR_USER_EQUATES:
		::g_EquateListLogic->Clear(*::g_Workspace->GetUserEquateList());
		return;

	case ID_EQUATES_EDIT_USER_EQUATES:
		::g_EquateListLogic->Edit(*g_Workspace->GetUserEquateList(), true, L"");
		return;

	case ID_EQUATES_DEFINE_USER_ADDRESS_RANGE:
		::g_EquateListLogic->DefineUserAddressRange(*::g_Workspace, L"");
		return;

	case ID_EQUATES_OPEN_USER_EQUATES:
		::g_EquateListLogic->LoadUserEquates(*::g_Workspace);
		return;

	case ID_EQUATES_SAVE_USER_EQUATES:
		::g_EquateListLogic->Save(*::g_Workspace->GetUserEquateList(), false);
		return;

	case ID_EQUATES_EXPORT_USER_EQUATES: // TODO: Select type (XASM/normal) rather in Save Dialog
		::g_EquateListLogic->Save(*::g_Workspace->GetUserEquateList(), true);
		return;
	}
}
