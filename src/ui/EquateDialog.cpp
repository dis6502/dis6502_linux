...
bool EquateDialog::ProcessCommand(COMMAND command, WPARAM wParam, LPARAM lParam) {
    int itemCount = 0;
...
bool EquateDialog::InitDialog() {
    ...
    if (!address.empty()) {
        ...
        return false; // Keep explicitly the just set keyboard focus
    }
    return true;
}

bool EquateDialog::OnOK() {
    SendMessage(hDlg, WM_COMMAND, IDC_ADD_EQUATE, 0L);

    equateList->Clear();
    auto listBox = GetListBox(IDC_EQUATE_LIST);
    const auto itemCount = listBox.GetCount();
    for (int index = itemCount - 1; index >= 0; index--) {
        auto itemText = listBox.GetStringAtIndex(index);
